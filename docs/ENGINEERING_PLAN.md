...
The pilot is developed and integration-tested in Google Colab, then deployed to a
dedicated production machine. Every phase below must preserve that portability:
algorithms live in the Python package, notebooks only orchestrate the CLI and
visualization, and environment-specific storage or hardware settings remain
outside core processing. The concrete cross-environment contract and migration
checklist are defined in [`PILOT_ARCHITECTURE.md`](PILOT_ARCHITECTURE.md).
